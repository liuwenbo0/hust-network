...
int main(){
	...
	*/
	...
	char root_adir[100];
	FILE *file;
	file = fopen("D:\\_Code\\C\\setting.txt","r");
    if (file == NULL) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        // 根据变量名赋值
        if (strncmp(line, "serverAddr", 10) == 0) {
            strncpy(serverAddr, value, sizeof(serverAddr));
			// 替换换行符为0,否则转化为二进制地址会失败
			for (int i = 0; i < strlen(serverAddr); i++) {
				if (serverAddr[i] == '\n') {
					serverAddr[i] = 0;
				}
   			}
        } 
		else if (strncmp(line, "serverPort", 10) == 0) {
            sscanf(value, "%d", &serverPort);
        }
		else if(strncmp(line, "root_adir", 9) == 0){
			memset(root_adir,0,sizeof(root_adir));
			strncpy(root_adir, value, sizeof(root_adir));
			for (int i = 0; i < strlen(root_adir); i++) {
				if (root_adir[i] == '\n') {
					root_adir[i] = 0;
				}
   			}
	...
	while(true){
		...
		//如果first_connection为true，sessionSocket还没有产生
		if (!first_connection) {
			...
			nTotal = select(0, &rfds, &wfds, NULL, NULL);
				...
				if (FD_ISSET(sessionSocket, &rfds)) {
					...
					if (rtn > 0) {
						if (strncmp(recvBuf, "GET", 3) == 0) {
							...
							line_content = strtok(recvBuf, "\r\n");
							while(line_num>0){
								printf("%s\n", line_content);
								line_content = strtok(NULL, "\r\n");
        						line_num--;
							}
							//printf("Received %d bytes from client: %.47s\n", rtn, recvBuf);
							// 解析HTTP请求，获取请求的文件路径
							sscanf(recvBuf, "GET /%s", file_name);
							strcpy(file_path, root_adir); // 先将rootadir复制到filepath中
							strcat(file_path, file_name); // 再将filename追加到filepath的末尾
							printf("client want to GET %s\n",file_path);
							// 打开请求的文件
							htmlfile = fopen(file_path, "rb");
							if (htmlfile == NULL) {
								printf("Failed to open file: %s\n", file_path);
								strcpy(file_path, "D:\\_Code\\C\\tip.html");
								htmlfile = fopen(file_path, "rb");
								// 构造HTTP响应
								memset(response_buffer, 0, sizeof(response_buffer));
								sprintf(response_buffer, "HTTP/1.1 400 Bad Request\r\n\r\n");
								// 发送HTTP响应头
								if (send(sessionSocket, response_buffer, strlen(response_buffer), 0) == SOCKET_ERROR) {
									printf("Failed to send response header: %s\n", strerror(errno));
									fclose(htmlfile);
									return 1;
								}
								// 发送文件内容
								while ((rtn = fread(response_buffer, 1, sizeof(response_buffer), htmlfile)) > 0) {
									if (send(sessionSocket, response_buffer, rtn, 0) == SOCKET_ERROR) {
										printf("Failed to send file content: %s\n", strerror(errno));
										break;
									}
								}
							}
